...
class TestReservationViewController(TestCase):
    def setUp(self):
        self.app = create_app('test')
        self.client = self.app.test_client()

    def test_get_reservations_endpoint(self):
    ...
    def test_get_reservations_with_filters(self):
        response = self.client.get('/database/reservations?reservation_id=1')
        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertEqual(response.json["code"], DatabaseResponseStatus.OK.get_value())
        self.assertEqual(response.json["code_message"], DatabaseResponseStatus.OK.get_description())

    def test_get_reservations_with_invalid_filters(self):
        response = self.client.get('/database/reservations?reservation_id=>1')
        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertEqual(response.json["code"], DatabaseResponseStatus.OK.get_value())
        self.assertEqual(response.json["code_message"], DatabaseResponseStatus.OK.get_description())

    def test_get_reservations_with_no_results(self):
        response = self.client.get('/database/reservations?reservation_id=!1')
        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertEqual(response.json["code"], DatabaseResponseStatus.OK.get_value())
        self.assertEqual(response.json["code_message"], DatabaseResponseStatus.OK.get_description())

    def test_get_reservations_with_invalid_filter(self):
        response = self.client.get('/database/reservations?reservation_id=-3')
        self.assertEqual(response.status_code, HTTPStatus.OK)
