...
class TestUserViewController(TestCase):
    def setUp(self):
    ...
    def test_get_users_with_filters(self):
        response = self.client.get('/database/users?user_id=1')
        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertEqual(response.json["code"], DatabaseResponseStatus.OK.get_value())
        self.assertEqual(response.json["code_message"], DatabaseResponseStatus.OK.get_description())

    def test_get_users_with_invalid_filters(self):
        response = self.client.get('/database/users?user_id=>3')
        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertEqual(response.json["code"], DatabaseResponseStatus.OK.get_value())
        self.assertEqual(response.json["code_message"], DatabaseResponseStatus.OK.get_description())

    def test_get_users_with_no_results(self):
        response = self.client.get('/database/users?user_id=!1')
        self.assertEqual(response.status_code, HTTPStatus.OK)
        self.assertEqual(response.json["code"], DatabaseResponseStatus.OK.get_value())
        self.assertEqual(response.json["code_message"], DatabaseResponseStatus.OK.get_description())

    def test_get_users_with_invalid_filter(self):
        response = self.client.get('/database/users?user_id=-3')
        self.assertEqual(response.status_code, HTTPStatus.OK)
