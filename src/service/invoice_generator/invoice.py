...
class InvoiceGenerator:
    def __init__(self, reservation_id, tax):
        self.reservation = ReservationView.from_id(reservation_id)
        self.tax = tax
        ...
        self.__invoice_view_details = InvoiceView.get_invoice_details_for_single_reservation(reservation_id)

        self.__customer_details =CustomerView.get_details_for_single_reservation(self.reservation.reservation_customer_id)

        self.__service_details = ServiceView.get_details_for_single_reservation(self.reservation.reservation_id)

        self.__reservation_details = db.session.query(
            ReservationView.reservation_id.label(),
        ...
        self.reservation_id = self.__reservation_details.reservation_id


        for room in self.__reservation_details:
            self.__gross_prices.append(room.duration.days * room.gross_price_room)

            self.__net_prices.append(self.__gross_prices[-1])

            self.__rooms_details.append([room.room_id,
                                         f'Adults: {room.number_of_adults}, children: {room.number_of_children}',
                                         room.duration.days,
                                         round(room.gross_price_room * self.tax_decimal, 2),
                                         self.__net_prices[-1]]
                                        )
